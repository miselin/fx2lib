...
__xdata BYTE buffa[64];
__xdata BYTE buffb[64];

/*
void test_buffers(BYTE a, BYTE b, BYTE c) {
	buffer_add_byte(a);
	buffer_add_byte(b);
	buffer_add_byte(c);

	IOA = buffer_remove_byte();
	IOB = buffer_remove_byte();
	IOC = buffer_remove_byte();
}*/

void test_autoptrs() {
	autoptr_memcpy(&buffa, &buffb, sizeof(buffa));
	autoptr_memcpy_inline(&buffa[0], &buffb[0], sizeof(buffa));
	autoptr_memcpy_asm(&buffa, &buffb, sizeof(buffa));
	dptr_memcpy(buffa, buffb, 64); //sizeof(buffa));
}

void main() {
	//test_buffers(0xa, 0x11, 0xbd);
	test_autoptrs();
