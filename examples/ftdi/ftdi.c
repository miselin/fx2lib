...
void execute_mpsse_request(struct ftdi_mpsse_request* request) {

	// B
	if (MPSSE_BITMODE & request.cmd) {
	} else {
	}

}

void execute_mpsse_bits(struct ftdi_mpsse_data_shift_command cmd, BYTE bits, BYTE length) {
	...
	// #############################################
	// If pin_data_out/pin_data_in bit capable
	// #############################################
